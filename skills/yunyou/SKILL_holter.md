...
description: 用于分析用户holter数据的数据库模式和业务逻辑，包括用户、holter和报告信息。可用于查询holter相关的数据库数据。
---
...
- algorithm_status （算法计算返回状态：-4=Out of Memory/-3=（算法) 程序异常 /-2=ECGData 原始文件不存在或格式错误 /-1 =
  检查软件授权 （License) 信息出错 / 0=ECGData 原始文件为空 / 1 = 处理成功 /）2=less - effective data/data error)
...
- rr_scatter_url （散点图信息，JSON 格式：url = 图片地址 /num = 未审核个数 /hasCheckNum = 已审核个数 /btFlag 类型 = 0 = 正常
  （N)/1 = 室上性 （S)/2 = 室性 （V)/26 = 房颤 （f)/-1 = 伪差 （-1)）
...
**算法计算完成**: is_uploaded =1 and ecg_cal_prog>=100 and hrv_cal_prog>=100，如果是2小时的还需要 and holter_type = 1 反之为
and holter_type != 1
