...
class ConfigurableSkillMiddleware(AgentMiddleware):
    ...
    _dir_skill_cache: dict[str, dict] = {}
    ...
    def __init__(self, skill_source: Union[str, List[Skill]]):
        ...
        if isinstance(skill_source, str):
            current_mtime = self.get_dir_mtime(skill_source)
            with self._cache_lock:
                cache_entry = self._dir_skill_cache.get(skill_source)
                if cache_entry and cache_entry.get("mtime", 0) >= current_mtime:
                    self.skills = [dict(item) for item in cache_entry["skills"]]
                else:
                    action = "重新大纲热加载" if cache_entry else "首次从目录加载"
                    log.info(f"MIDDLEWARE: {action} 技能: {skill_source}", target=LogTarget.ALL)
                    loaded_skills = SkillLoader.load_from_dir(skill_source)
                    self._dir_skill_cache[skill_source] = {
                        "mtime": current_mtime,
                        "skills": [dict(item) for item in loaded_skills]
                    }
