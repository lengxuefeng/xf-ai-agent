...
MONGO_URI = "mongodb://root:xiaoleng@192.168.1.10:27017/xf-ai-agent"
...
# ================= 核心：自动建表 =================
try:
    # 从新的 PgSQL 引擎入口导入 Base
    # 显式导入所有 Model，触发 SQLAlchemy 的注册机制
    import models.user_info
    import models.model_setting
    import models.user_model
    import models.user_mcp

    # 如果 chat_history 已经改成了 PgSQL 版，一并导入
    try:
        import models.chat_history
    except ImportError:
        pass

    print("🐘 正在 PostgreSQL 中自动创建表结构...")
    Base.metadata.create_all(bind=pg_engine)
    print("✅ 表结构初始化完成！\n")
except Exception as e:
    print(
        f"⚠️ 导入模型或建表失败，请检查 app/models 里的文件是否把 from db import Base 改成了 from db import Base。报错详情: {e}")
    sys.exit(1)


def reset_pg_sequence(pg_conn, table_name, pk_column="id"):
    """【黑科技】修复 PgSQL 手动插入数据后的自增序列不同步问题"""
    try:
    ...
    with pg_engine.begin() as pg_conn:
        with mysql_engine.connect() as mysql_conn:
            for table_name in tables_to_migrate:
                ...
                print(f"      ✅ 成功迁移 {len(rows)} 条记录 (序列已对齐)")
