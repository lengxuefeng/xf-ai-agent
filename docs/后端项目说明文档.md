...
- **API层（FastAPI）**：接收请求、鉴权、SSE 输出（`async def` 路由，真正逐块推送）。
- **服务层（ChatService）**：处理历史消息、启动 GraphRunner、落库聊天结果（阻塞 DB 操作通过 `asyncio.to_thread` 不阻塞事件循环）。
- **图执行层（GraphRunner + Supervisor）**：调度路由、执行子图、处理中断恢复（`asyncio.Queue` 桥接后台线程，`SessionPool` 预热池消除首次编译延迟）。
...
## 10. 本轮去硬编码与封装优化（2026-03-08 增量）

### 10.1 新增常量中心

- `app/constants/chat_service_constants.py`
  - 聊天服务默认模型配置
  - SSE 错误提示模板
  - 统一流式响应头
- `app/constants/workflow_constants.py`
  - GraphRunner 线程队列事件类型（`log/graph/done/error`）
  - 图流模式常量（`updates/messages`）
- `app/constants/sse_constants.py`
  - 新增 `LOG` 事件类型
  - 新增 SSE 负载字段名常量（`type/content`）
- `app/constants/agent_messages.py`
  - Search/Weather 循环保护提示
  - Yunyou 审批提示
  - GraphRunner 拒绝反馈提示
- `app/constants/yunyou_keywords.py`
  - 新增相对日期关键词集
  - 新增报告状态/holter类型筛选关键词映射
  - 新增 Holter 表格列定义

### 10.2 核心代码改造点

1. `ChatService`  
   - 抽取统一 `_format_error_event`，所有异常输出走同一 SSE 结构。
   - 默认模型参数改为引用常量中心，减少函数签名硬编码。
   - 正文提取类型统一走 `CHAT_AI_CONTENT_TYPES`。
2. `GraphRunner`  
   - 队列事件不再使用裸字符串，统一使用 `GraphQueueItemType`。
   - 图流模式不再写死，改为 `GRAPH_STREAM_MODES`。
   - SSE payload 字段由常量控制，避免协议字段散落。
3. `YunyouAgent`  
   - 相对日期词、报告状态词、Holter 类型词、表格列全部词典化。
   - 审批提示文案改为常量引用。
4. `search_agent/weather_agent`  
   - 工具循环上限触发提示改为常量引用。
5. 日志与事件组件  
   - `callback_handler/custom_logger/sse_logging_handler/graph_runner_events` 统一 `log` 事件类型来源。
   - `sse_logging_handler` 删除重复定义的 `create_sse_handler`，避免维护歧义。
6. 审批服务  
   - `InterruptService` 的处理状态 `processed` 改为常量中心维护。

### 10.3 回归建议（本轮改造后必测）

1. `POST /api/v1/chat/stream` 异常时，前端可见统一错误卡片。  
2. `interrupt -> approve/reject -> resume` 链路可完整闭环。  
3. Yunyou “最近/本周/上月/待审核/48小时” 等关键词仍可正确命中筛选。  
4. 搜索/天气工具循环到阈值后能返回“可读可恢复”的最终提示。  
...
1. 用户先说“天气不好”，Agent 追问“哪个城市”。  
2. 用户只回复“郑州”，系统误路由成 `search_agent`，输出城市百科。  
3. 下一轮又重复追问城市。  

### 11.2 根因

1. 路由器未把“仅城市名”识别为补充参数。  
2. 历史意图继承只覆盖 holter/sql，未覆盖 weather/search。  
3. 子 Agent 执行默认只带当前一句，未带最近多轮上下文。  

### 11.3 已落地修复

1. `supervisor` 新增天气域识别与“城市补充句”识别。  
2. 历史意图继承扩展到 `weather_agent/search_agent`。  
3. `single_agent_node` 调用子 Agent 时携带最近 `N` 条上下文（`AGENT_CONTEXT_HISTORY_MESSAGES`）。  
4. `BaseAgent.run` 支持从 `req.state.messages` 注入历史消息，避免重复追问。  
5. 历史注入前增加压缩：优先 LangChain 原生 `trim_messages`，失败时回退字符预算裁剪，降低 token 成本。  
...
2. 系统：`请问在哪个城市`  
3. 用户：`郑州`  
4. 预期：直接给郑州天气，不再输出城市百科，也不重复追问城市。  

### 11.5 历史压缩配置（新增）

- `AGENT_CONTEXT_HISTORY_MESSAGES`：参与压缩的最近消息条数（默认 `10`）  
- `AGENT_CONTEXT_COMPRESS_MAX_TOKENS`：历史压缩 token 上限（默认 `1800`）  
- `AGENT_CONTEXT_COMPRESS_MAX_CHARS`：压缩回退字符预算（默认 `12000`）  

### 11.6 主题跑偏修复（搜索误答天气）

问题：用户问“小区/房价/酒店周边”，偶发返回天气播报。  

修复：

1. `supervisor` 增加“指代补充句”识别（如“后面那个小区”），优先继承上一轮 `search_agent` 意图。  
2. `search_agent` 增加主题护栏：若用户是房产主题且模型输出天气字段（温度/湿度/风向等），直接拦截并纠偏。  
3. `BaseAgent` 历史注入增加词面相关性筛选，减少旧天气上下文污染。  
