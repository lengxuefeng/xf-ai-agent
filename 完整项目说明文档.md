...
| LangChain Core | 0.3.10+ | LangChain 核心组件 |
...
| langchain-ollama | 0.2.0+ | 本地 Ollama 模型 |
| langchain-community | 0.3.0+ | 社区模型集成 |
| langchain-mcp-adapters | 0.2.0+ | MCP 协议适配 |
...
| langchain-core | 0.3.10+ | LangChain 核心组件 | 提示词、消息、工具等 |
| langchain-openai | 0.2.0+ | OpenAI 兼容接口 | OpenAI、智谱、硅基流动等 |
| langchain-google-genai | 2.0.0+ | Google Gemini | Google Gemini 模型 |
| langchain-community | 0.3.0+ | 社区扩展 | Ollama、Tavily 等社区工具 |
| langchain-ollama | 0.2.0+ | Ollama 集成 | 本地 Ollama 模型 |
| langchain-tavily | 0.0.1+ | Tavily 搜索 | 联网搜索功能 |
| langgraph | 0.2.50+ | 状态机引擎 | 构建复杂的多步骤应用 |
| langsmith | 0.1.130+ | LangSmith 追踪 | 可视化执行过程 |
| langchain-mcp-adapters | 0.2.0+ | MCP 协议适配 | Model Context Protocol 支持 |
| langchain-postgres | 0.0.17+ | PostgreSQL 向量存储 | RAG 向量数据库 |
...
| langgraph-checkpoint-postgres | 3.0.4+ | LangGraph 检查点 | 状态持久化 |
...
| langgraph-cli | 0.4.12+ | LangGraph CLI | LangGraph 命令行工具 |
